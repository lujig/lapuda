...
class ld():
	...
	#
	def read_chan(self,chan_num):	# read the data in specific channel index in LD file
		ndata_chan=np.int64(np.array(self.__size__[2:]).prod())
		if chan_num>self.__size__[1]:
			raise Exception('The input channel number is larger than total channel number of file.')
		self.file=open(self.name,'rb')
	...
	#
	def read_data(self):	# read all data in LD file
	...
	def write_period(self,data,p_num):	# write data into specific sub-integration index in LD file
		...
		self.__refresh_size__()
	#
	def read_period(self,p_num):	# read the data in specific sub-integration index in LD file
		ndata_period=np.int64(self.__size__[3]*self.__size__[4])
		ndata_chan=np.int64(ndata_period*self.__size__[2])
		if p_num>self.__size__[2]:
			raise Exception('The input period number is larger than total period number of file.')
	...
	def __write_chanbins__(self,data,bin_start,chan_num):	# write the data series into specific frequency channel of LD file at specified starting bin index
		...
		self.__refresh_size__()
	#
	def __read_bin_segment__(self,bin_start,bin_num):	# read data segment with specified starting bin index and total bin numbers in all frequency channels
		ndata_chan=np.int64(np.array(self.__size__[2:]).prod())
		...
		return data
	#
	def chan_scrunch(self,select_chan=[],start_period=0,end_period=0,chan_weighted=True):	# scrunch the data in LD file along frequency axis
		ndata_chan=np.int64(np.array(self.__size__[2:]).prod())
		...
		self.file=open(self.name,'rb')
		info=self.read_info()
		if chan_weighted and 'chan_weight' in info.keys(): 
			weight=self.read_para('chan_weight')
		...
		return data
	#
	def period_scrunch(self,start_period=0,end_period=0,select_chan=[]):	# scrunch the data in LD file along subint axis
		ndata_chan=np.int64(np.array(self.__size__[2:]).prod())
		if len(select_chan)==0:
			select_chan=np.arange(self.__size__[1])
		elif not set(select_chan).issubset(set(np.arange(self.__size__[1]))):
