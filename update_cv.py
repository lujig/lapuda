...
def update_leap():
	...
	for i in range(lenb):
		...
		if t:
			...
			b.pop(num)
	f=open(dirname+'/materials/'+'leap.txt','w')
	f.writelines(b)
	f.close()
#
def show_leaptxt():
	f=open(dirname+'/materials/'+'leap.txt')
	s=f.read()
	f.close()
	print(s)
#
...
def update_polarmotion():
	# utc2ut1
	a=ur.urlopen('https://hpiers.obspm.fr/eoppc/eop/eopc04/eopc04.1962-now').readlines()
	b=list(map(lambda x:x.strip().decode(),a))
	lenb=len(b)
	for i in range(lenb):
		num=lenb-1-i
		...
		if t:
			...
			b.pop(num)
	f=open(dirname+'/materials/'+'eopc.txt','w')
	f.writelines(b)
	f.close()
#
def show_polarmotion(ax):
	mjd,dx,dy=np.loadtxt(dirname+'/materials/'+'eopc.txt')[:,4:7].T
...
#
def show_utc2ut1(ax):
	mjd,dt=np.loadtxt(dirname+'/materials/'+'eopc.txt')[:,[4,7]].T
	ax.plot(mjd,dt)
	return 'MJD','UT1-UTC (s)'
#
def show_pmxy(ax):
	dx,dy=np.loadtxt(dirname+'/materials/'+'eopc.txt')[:,4:6].T
	ax.plot(dx,dy)
	return 'PMDX (\'\')','PMDY (\'\')'
#
def update_tai2ut1():
	...
	for i in range(lenb):
		...
		if t:
			...
			b.pop(num)
	mjd,deltat=np.float64(list(map(lambda x:x.split(),b)))[:,[4,7]].T
	...
	deltat+=leap_time-10
	f=open(dirname+'/materials/'+'tai2ut1.txt','w')
	f.writelines(list(map(lambda x: str(x[0])+' '+str(x[1])+' '+str(x[2])+'\n',zip(mjd,taimjd,deltat))))
	f.close()
#
def show_tai2ut1(ax):
	mjd,dut1=np.loadtxt(dirname+'/materials/'+'tai2ut1.txt')[:,[0,2]].T
	ax.plot(mjd,dut1)
...
def update_tai2tt():
	...
	for i in range(lenb):
		...
		if t:
			...
			b.pop(num)
	f=open(dirname+'/materials/'+'tai2tt.txt','w')
	f.writelines(b)
	f.close()
#
def show_tai2tt(ax):
...
def update_gps2utc():
	...
	for i in range(lenb):
		...
		if t:
			...
			b.pop(num)
	f=open(dirname+'/materials/'+'gps2utc.txt','w')
	f.writelines(b)
	f.close()
#
def show_gps2utc(ax):
